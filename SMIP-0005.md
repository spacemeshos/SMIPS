...
- Note that we currently mix between 3 kind of concerns in a single enum we call "transaction status":
  1. The transaction status on its journey to them mesh the mesh via a full node, a transaction pool and a block.
  1. The status of the layer which includes a block with the transaction (confirmed, approved, etc...)
  1. The actual transaction processing status as resulted from execution of this transaction.
