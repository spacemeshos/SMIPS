...
1. Wallets need to be smart enough to inspect more than one transaction receipt for a transaction and display to the user the relevant information.
1. Can transaction receipts be pruned? Pruning transaction receipts means loss of ability of wallets to display old transaction execution results. Perhaps only archive nodes keep all transaction receipts and full nodes prune them to reduce storage costs?
